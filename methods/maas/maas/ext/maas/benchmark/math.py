...
class MATHBenchmark(BaseBenchmark):
    ...
    async def _generate_output(self, graph, input_text):
        result = await asyncio.wait_for(graph(input_text), timeout=1500)
    ...
    async def evaluate_problem(self, problem: dict, graph: Callable):
        ...
        try:
            output, cost, logprob, token_stats = await self._generate_output(graph, input_text)
            if not output:
            ...
            # Include token statistics in return value
            return input_text, output, expected_output, uni_score, cost, logprob, token_stats

        except Exception as e:
            logger.info(f"Maximum retries reached. Skipping this sample. Error: {e}")
            return input_text, str(e), expected_output, 0.0, 0.0, torch.tensor(0.0, dtype=torch.float32, device=self.device), None
        
    def get_result_columns(self) -> List[str]:
        return ["question", "prediction", "expected_output", "score", "cost", "logprob", "token_stats"]
