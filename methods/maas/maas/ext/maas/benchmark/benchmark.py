...
class BaseBenchmark(ABC):
    ...
    def save_results_to_csv(self, results: List[Tuple[Any, ...]], columns: List[str]):
        ...
        # Drop columns that shouldn't be saved to CSV
        columns_to_drop = []
        if "cost" in df.columns:
            columns_to_drop.append("cost")
        if "token_stats" in df.columns:
            columns_to_drop.append("token_stats")  # Token stats are complex objects, not suitable for CSV
        if columns_to_drop:
        ...
        for rep in range(1, repetitions + 1):
            ...
            for batch_start in range(0, len(data), self.batch_size):
                ...
                for r in batch_results:
                    # Handle both old (6 values) and new (7 values) result formats
                    if len(r) >= 7:
                        logprob = r[5]
                        cost = r[4]
                        score = r[3]
                        # token_stats = r[6]  # Available if needed
                    else:
                        # Fallback for old format
                        logprob = r[5] if len(r) > 5 else torch.tensor(0.0)
                        cost = r[4] if len(r) > 4 else 0.0
