...
def get_client():
    """Lazy load client to avoid module-level config loading."""
    global _llm_config, _client
    if _client is None:
        try:
            _llm_config = load_llm_config(default_model)
        except (FileNotFoundError, ValueError) as e:
            print(f"[TextGrad] Config loading failed, using fallback: {e}")
            # Use fallback config
            _llm_config = {
                "api_key": "placeholder",
